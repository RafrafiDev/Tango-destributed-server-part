...
pcl::PointCloud<pcl::PointXYZRGB>::Ptr
readFile(std::string fileName,float*xxx,float* yyy,float* zzz,float*xx,float* yy,float* zz,float* ww){
...
pcl::PointCloud<pcl::PointXYZRGB>::Ptr res(new PointCloud());
    ...
    if (myfile.is_open()) {
        ...
        getline (myfile, line);
        sscanf (line.c_str(),"%f;%f;%f;%f",xx,yy,zz,ww);
        getline (myfile, line);
...
float xx, yy,zz,ww,posx,posy,posz;
pcl::PointCloud<pcl::PointXYZRGB>::Ptr point_cloud_ptr = readFile("output4.csv",&posx,&posy,&posz,&xx,&yy,&zz,&ww),cloud_f (new pcl::PointCloud<pcl::PointXYZRGB>);
std::cout<<"orientation "<<xx<<" "<<yy<<" "<<zz<<" "<<ww<<std::endl;


Eigen::Vector3f point_on_line(posx,posy,posz), line_direction(0,0,1);
Eigen::Vector4f quat(ww,xx,yy,zz);
line_direction=quaternionMutltiply(quat,line_direction);
