...
void unittreatment(int *newsockfd,bool *update, boost::shared_ptr<pcl::visualization::PCLVisualizer> * viewer){
 ...
 while(1){
         ...
         if(read(*newsockfd,delimiter,1)<0)continue;
       ...
       char* buffer=new char[buffer_point_size];
  ...
  // pcl::PointCloud<pcl::PointXYZRGB>::Ptr point_cloud_ptr =convertBytesToPointCloud(buffer,buffer_point_size);
   ...
   //pcl::visualization::PointCloudColorHandlerRGBField<pcl::PointXYZRGB> rgb(point_cloud_ptr);
     // (*viewer)->addPointCloud<pcl::PointXYZRGB> (point_cloud_ptr, rgb, "sample cloud");
       ...
       if (write(*newsockfd,"I got your message",18) < 0) error("ERROR writing to socket");

    *update=true;
       //client need to close if he wants to
    //close(*newsockfd);
...
void obstacle_detection(pcl::PointCloud<pcl::PointXYZRGB>::Ptr cloud_filtered){
